const app = {
    ...
    async loadPlayerDetail() {
        ...
        // Load inventory inspector in background
    ...
    async loadInventoryInspector() {
        ...
        if (res.inventory) {
            ...
            document.getElementById('inv-snapshot-time').innerText = `Snapshot: ${t}`;
        }
    },
    ...
    buildTooltip(item) {
        ...
        const displayName = item.name || item.id?.replace(/_/g, ' ') || 'Unknown';
        html += `<span class="mc-name">${this.mcToHtml(nameColor + '&l' + displayName)}</span>`;
        ...
        if (item.lore?.length) {
            item.lore.forEach(line => {
                if (!line || line === '') {
                    ...
                    html += `<span class="mc-lore">${this.mcToHtml(line)}</span>`;
                }
            });
        }

        // Stats from stats object if lore is missing
        if (item.stats && Object.keys(item.stats).length > 0) {
            html += `<span class="mc-empty"></span>`;
            const s = item.stats;
            if (s.health)   html += `<span class="mc-lore">${this.mcToHtml(`&7Health: &c+${s.health} ❤`)}</span>`;
            if (s.defense)  html += `<span class="mc-lore">${this.mcToHtml(`&7Defense: &a+${s.defense}`)}</span>`;
            if (s.speed)    html += `<span class="mc-lore">${this.mcToHtml(`&7Speed: &f+${s.speed} ⚡`)}</span>`;
            if (s.damage)   html += `<span class="mc-lore">${this.mcToHtml(`&7Damage: &c+${s.damage}`)}</span>`;
            if (s.strength) html += `<span class="mc-lore">${this.mcToHtml(`&7Strength: &c+${s.strength}`)}</span>`;
            if (s.farming_fortune)   html += `<span class="mc-lore">${this.mcToHtml(`&7Farming Fortune: &6+${s.farming_fortune} ☘`)}</span>`;
            if (s.enchanted_chance)  html += `<span class="mc-lore">${this.mcToHtml(`&7Enchanted Chance: &a+${s.enchanted_chance}%%`)}</span>`;
        }
        ...
        const itype = item.item_type && item.item_type !== 'none' ? ` &7✦ ${tc}&l${item.item_type}` : '';
        html += `<span class="mc-lore">${this.mcToHtml(`${tc}&l${item.tier}${itype}`)}</span>`;
        if (item.amount > 1) html += `<span class="mc-lore" style="color:#aaaaaa;">Amount: ${item.amount}</span>`;
        return html;
    },
    ...
    makeSlot(item, extraClass = '') {
        ...
        slot.className = `inv-slot tier-${tier} ${extraClass}`;

        // Item icon via mc-heads
        ...
        // Enchant glow
        if (item.enchanted === true || item.enchanted === 'true') {
            const glow = document.createElement('div');
            glow.className = 'slot-enchant-glow';
    ...
    async loadInvTimeline() {
        ...
        if (!res?.length) {
            el.innerHTML = `<p style="font-family:'Space Mono',monospace;font-size:11px;color:var(--text-dim);padding:16px 0;">// No snapshots yet.</p>`;
            return;
        }
        const triggerIcon = { quit: '🚪', periodic: '⏱', admin: '🛡' };
        el.innerHTML = res.map(s => {
            const date = new Date(s.snapped_at).toLocaleString('en-GB', { hour12: false });
            const icon = triggerIcon[s.trigger_type] || '📦';
            return `<div class="inv-timeline-entry" onclick="app.loadInvSnapshot(${s.id})">
                <span style="font-size:16px;">${icon}</span>
                <div style="flex:1;">
                    <div style="font-family:'Space Mono',monospace;font-size:11px;font-weight:700;">${date}</div>
                    <div style="font-family:'Space Mono',monospace;font-size:9px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;">${s.trigger_type}</div>
                </div>
    ...
    async loadInvSnapshot(id) {
        ...
        if (res.error) return this.toast(res.error, 'error');
        this._invData = res;
        this.setInvTab('inventory', document.querySelectorAll('.inv-tab')[0]);
        this.renderInventoryTab();
        const t = new Date(res.snapped_at).toLocaleString('en-GB', { hour12: false });
        document.getElementById('inv-snapshot-time').innerText = `Snapshot: ${t} (${res.trigger_type})`;
        this.toast(`Viewing snapshot from ${t}`);
    },
    ...
    async loadRollbackTimeline() {
        ...
        // Build a map of closest inventory snapshot per stat snapshot
        const findClosestInvSnap = (snapDate) => {
        ...
        if (!snapshots.length) {
            ...
            snapshots.forEach((snap, i) => {
                ...
                const closestInv = findClosestInvSnap(snap.snapped_at);
                const invBadge = closestInv
                    ? `<span style="font-family:'Space Mono',monospace;font-size:9px;background:var(--accent-glow);color:var(--accent-bright);border:1px solid rgba(91,106,255,0.3);padding:1px 6px;border-radius:2px;cursor:pointer;" onclick="app.loadInvSnapshot(${closestInv.id})" title="View inventory at this time">📦 INV</span>`
                    : '';
                ...
                html += `<div style="display:flex;align-items:flex-start;gap:16px;padding:10px 0;position:relative;">
                    ...
                    <div style="flex:1;background:var(--bg-raised);border:1px solid ${isCurrent ? 'rgba(0,229,160,0.2)' : 'var(--border-dim)'};border-radius:4px;padding:12px 16px;">
                        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                            <div>
                                <div style="font-family:'Space Mono',monospace;font-size:10px;color:var(--text-dim);margin-bottom:4px;">${date} ${invBadge}</div>
                            ...
                            ${!isCurrent
                                ? `<div style="display:flex;gap:6px;">
                                    ${closestInv ? `<button class="btn-sm" onclick="app.executeRollback(${snap.id},'${player.name}',${snap.coins},${snap.level},${closestInv.id})" title="Restore stats + inventory">RESTORE +INV</button>` : ''}
                                    <button class="btn-sm btn-red" onclick="app.executeRollback(${snap.id},'${player.name}',${snap.coins},${snap.level})">RESTORE</button>
                                   </div>`
                                : `<span style="font-family:'Space Mono',monospace;font-size:9px;color:var(--green);background:var(--green-dim);padding:2px 8px;border-radius:2px;border:1px solid rgba(0,229,160,0.2);">CURRENT</span>`}
                        </div>
                    </div>
                </div>`;
            });
    ...
    async executeRollback(snapshotId, name, coins, level, invSnapshotId = null) {
        ...
        const ok = await this.confirm(
            `Roll back ${name} to: ${this.fmtCoins(coins)} coins, Level ${level}?${hasInv ? '\n\n+ Inventory will also be restored.' : '\n\nStats only — no inventory restore.'}\n\nThis cannot be undone.`,
            true
        );
        if (!ok) return;
        ...
        const invMsg = res.inv_restored ? ' + inventory' : '';
        this.toast(`${name} rolled back — Coins: ${this.fmtCoins(res.restored.coins)} ◎${invMsg}`);
        this.loadRollbackTimeline();
        this.loadLogs();
    },
